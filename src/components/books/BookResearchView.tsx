    ...
    <div className="space-y-6 pb-12 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Search by Keyword, ASIN, or Author to uncover estimated sales, BSR trends, and pricing strategies.
        </p>
      </div>
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex gap-3 text-indigo-900 text-sm">
        ...
        <div>
          <strong className="font-semibold block mb-1">Methodology Disclaimer</strong>
          <p className="text-indigo-800/80 leading-relaxed text-xs">
            Sales and revenue figures are clearly labeled as <strong>Estimated</strong>. They are calculated using proprietary algorithms analyzing Best Seller Rank (BSR), category momentum, historical trends, and market sizing models. They do not represent exact or guaranteed historical figures from Amazon.
          </p>
        </div>
      </div>
      ...
      {!isLoading && results.length > 0 && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {results.map((book) => {
            ...
            return (
              <div key={book.asin} className="bg-white border border-slate-200 rounded-xl hover:border-indigo-300 hover:shadow-md transition flex flex-col">
                <div className="p-5 flex-1">
                  <div className="flex justify-between items-start gap-4 mb-3">
                    <div>
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {book.categories.map((c, idx) => (
                          <span key={idx} className="text-[10px] font-bold uppercase tracking-wider text-slate-500 bg-slate-100 px-2 py-0.5 rounded">
                            {c}
                          </span>
                        ))}
                      </div>
                      ...
                      <p className="text-sm text-slate-600 mt-1 font-medium">by {book.author}</p>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-slate-500 mb-6">
                    <div className="flex items-center gap-1.5 font-mono">
                      <span className="font-semibold text-slate-700">ASIN:</span> {book.asin}
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Calendar className="h-3.5 w-3.5" />
                      {new Date(book.publishDate).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}
                    </div>
                    <div className="flex items-center gap-1.5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 border-t border-slate-100 pt-5">
                    <div>
                      <div className="text-[10px] uppercase font-bold text-slate-400 mb-1">BSR Rank</div>
                      <div className="text-sm font-semibold text-slate-800 flex items-center gap-1.5">
                        #{book.bestSellerRank.toLocaleString()}
                        {book.trend === 'Up' && <TrendingUp className="h-3.5 w-3.5 text-emerald-500" />}
                        {book.trend === 'Down' && <TrendingDown className="h-3.5 w-3.5 text-rose-500" />}
                        {book.trend === 'Flat' && <Minus className="h-3.5 w-3.5 text-slate-400" />}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase font-bold text-slate-400 mb-1">List Price</div>
                      <div className="text-sm font-semibold text-slate-800 flex items-center gap-1.5">
                        ${book.priceUSD.toFixed(2)}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase font-bold text-slate-400 mb-1">Est. Sales / Mo</div>
                      <div className="text-sm font-black text-indigo-700">
                        {book.estimatedMonthlySales.toLocaleString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase font-bold text-slate-400 mb-1">Est. Rev / Mo</div>
                      <div className="text-sm font-black text-emerald-700">
                        ${book.revenueEstUSD.toLocaleString()}
                      </div>
                    </div>
                  </div>
                </div>
