...
import { 
  Search, Swords, BarChart3, Info, ChevronDown, ChevronUp, TrendingUp, Tags, Shield, AlertCircle
} from 'lucide-react';
...
export const CompetitionAnalysisView: React.FC = () => {
  ...
  const performAnalysis = useCallback(async (e?: React.FormEvent) => {
    ...
    <div className="space-y-6 pb-12 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Understand how crowded a niche is, typical pricing, and what successful books look like.
        </p>
      </div>
      ...
      {isLoading && (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-700">Analyzing Market Data</h3>
          <p className="text-sm text-slate-500 mt-1">Calculating crowding, pricing models, and keyword overlap...</p>
        </div>
      )}
      ...
      {!isLoading && data && (
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div className={`p-8 border-b text-center ${levelColor(data.competitionLevel).replace('border', 'border-b')}`}>
              <h2 className="text-sm font-bold uppercase tracking-widest opacity-80 mb-2">Competition Level</h2>
              <div className="text-4xl sm:text-5xl font-black mb-4">{data.competitionLevel}</div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-slate-100 text-center">
              <div className="p-4">
                <div className="text-xs font-semibold text-slate-400 uppercase mb-1">Books Found</div>
                <div className="text-lg font-bold text-slate-800">{data.metrics.totalBooksFound.toLocaleString()}</div>
              </div>
              <div className="p-4">
                <div className="text-xs font-semibold text-slate-400 uppercase mb-1">Avg Price</div>
                <div className="text-lg font-bold text-slate-800">${data.metrics.avgPrice.toFixed(2)}</div>
              </div>
              <div className="p-4">
                <div className="text-xs font-semibold text-slate-400 uppercase mb-1">Avg Reviews</div>
                <div className="text-lg font-bold text-slate-800">{data.metrics.avgReviews.toLocaleString()}</div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-top-4 duration-500">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm md:col-span-2">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-bold text-slate-800">What Successful Books Look Like</h3>
                </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-slate-800 text-sm">Typical Pricing Distribution</h3>
                </div>
                  ...
                  {data.distributions.price.map((p, idx) => (
                    <div key={idx}>
                      ...
                      <div className="w-full bg-slate-100 rounded-full h-1.5">
                        <div className="bg-indigo-500 h-1.5 rounded-full" style={{ width: `${p.percentage}%` }}></div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  <BarChart3 className="h-4 w-4 text-slate-400" />
                  <h3 className="font-bold text-slate-800 text-sm">Rating Distribution</h3>
                </div>
                <div className="space-y-3">
                  {data.distributions.rating.map((r, idx) => (
                    <div key={idx}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="font-semibold text-slate-700">{r.range} Stars</span>
                        <span className="text-slate-500">{r.percentage}%</span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-1.5">
                        <div className="bg-amber-500 h-1.5 rounded-full" style={{ width: `${r.percentage}%` }}></div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-slate-800 text-sm">Publication Activity (Recent Years)</h3>
                </div>
                <div className="h-40 flex items-end justify-between gap-2 pt-4">
                  {data.distributions.publication.map((pub, idx) => {
                    const max = Math.max(...data.distributions.publication.map(p => p.count));
                    ...
                    return (
                      <div key={idx} className="flex-1 flex flex-col items-center justify-end group">
                        ...
                        <div 
                          ...
                          style={{ height: `${height}%` }}
                        ></div>
                        <div className="text-[10px] font-semibold text-slate-600 mt-2 text-center">{pub.year}</div>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-slate-800 text-sm">Keyword Overlap</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {data.topKeywords.map((kw, idx) => (
                    <div key={idx} className="px-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-xs flex items-center gap-2">
