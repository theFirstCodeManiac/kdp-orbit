...
interface IPaymentProvider {
  ...
  handleWebhook(req: Request): Promise<{ eventId: string; type: string; reference: string; status: string } | null>;
}
...
class PaystackProvider implements IPaymentProvider {
  ...
  async createCheckoutSession(userId: string, email: string, planId: string, billingCycle: string, amount: number, currency: string, successUrl: string, cancelUrl: string): Promise<PaymentCheckoutResult> {
    if (!process.env.PAYSTACK_SECRET_KEY) return this.mockCheckout(userId, planId, billingCycle, successUrl);
  ...
  async handleWebhook(req: Request): Promise<{ eventId: string; type: string; reference: string; status: string } | null> {
  ...
  private async mockCheckout(userId: string, planId: string, billingCycle: string, successUrl: string) {
...
paymentsRouter.post('/checkout', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const currency = user.preferredCurrency === 'NGN' ? 'NGN' : 'USD';
    const amountUSD = billingCycle === 'yearly' ? plan.yearlyPriceUSD : plan.monthlyPriceUSD;
    const amount = currency === 'NGN' ? amountUSD * 1500 : amountUSD; // Basic FX for NGN
    ...
    const { checkoutUrl, reference } = await provider.createCheckoutSession(
      userId, user.email, planId, billingCycle, amount, currency, successUrl, cancelUrl
    );
    ...
    res.status(500).json({ success: false, error: error.message || 'Checkout initialization failed' });
  }
});
...
paymentsRouter.post('/webhook/:provider', express.json({ type: 'application/json' }), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result) {
      ...
      if (tx && tx.status !== 'successful' && result.status === 'successful') {
        tx.status = 'successful';
        tx.updatedAt = new Date().toISOString();
        applySubscriptionUpgrade(tx);
      }
...
function applySubscriptionUpgrade(tx: DBPaymentTransaction) {
  ...
  if (!billingInfo) {
    billingInfo = {
      ...
      nextBillingDate: new Date(Date.now() + (tx.billingCycle === 'yearly' ? 365 : 30) * 86400000).toISOString().split('T')[0],
      invoices: []
    };
    ...
    billingInfo.nextBillingDate = new Date(Date.now() + (tx.billingCycle === 'yearly' ? 365 : 30) * 86400000).toISOString().split('T')[0];
  }
