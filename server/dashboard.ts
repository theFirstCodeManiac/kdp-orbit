...
import { db, DBUsageQuota, DBResearchItem, DBCoverProject, DBRecentSearch } from './db.ts';
...
export const TRENDING_OPPORTUNITIES = [
  {
    ...
    cpcUSD: 0.42,
    whyHot: 'Strong Q3-Q4 search surge; high TikTok engagement with low title saturation in large print format.'
  },
  {
    id: 'trend_02',
    ...
    cpcUSD: 0.28,
    whyHot: 'Underserved demographic in diaspora markets (US, UK, CA, NG) seeking authentic cultural motifs.'
  },
  {
    id: 'trend_03',
    ...
    cpcUSD: 0.38,
    whyHot: 'Loyal repeat buyers with high customer lifetime value and minimal price resistance at $12.99.'
  },
  {
    id: 'trend_04',
    ...
    cpcUSD: 0.65,
    whyHot: 'Male wellness movement expanding beyond generic journals into guided shadow exercises.'
  }
];
...
function getDefaultQuota(planId: string, userId: string): DBUsageQuota {
  ...
  return {
    ...
    month: currentMonth,
    keywordSearchesUsed: 0,
    keywordSearchesLimit: 20,
    nicheQueriesUsed: 0,
    nicheQueriesLimit: 10,
    aiCreditsUsed: 0,
    aiCreditsLimit: 15,
    coverExportsUsed: 0,
    coverExportsLimit: 3,
...
dashboardRouter.get('/overview', requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const billing = db.billingRecords.get(userId) || {
      ...
      planId: user.planId,
      status: 'active',
      currency: user.preferredCurrency === 'NGN' ? 'NGN' : 'USD',
      currentAmount: user.planId === 'author_pro' ? (user.preferredCurrency === 'NGN' ? 12500 : 19) : 0,
    ...
    // Sort recent first
    savedNiches.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    recentCovers.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    res.json({
      ...
      data: {
        ...
        subscription: {
          ...
          amount: (billing as any).currentAmount !== undefined ? (billing as any).currentAmount : (billing as any).currentMonthlyAmount || 0,
        ...
        usage: {
          ...
          keywordSearches: {
            ...
            remaining: Math.max(0, usage.keywordSearchesLimit - usage.keywordSearchesUsed),
            percent: Math.min(100, Math.round((usage.keywordSearchesUsed / usage.keywordSearchesLimit) * 100)),
          },
          nicheQueries: {
            used: usage.nicheQueriesUsed,
            limit: usage.nicheQueriesLimit,
            remaining: Math.max(0, usage.nicheQueriesLimit - usage.nicheQueriesUsed),
            percent: Math.min(100, Math.round((usage.nicheQueriesUsed / usage.nicheQueriesLimit) * 100)),
          },
          aiCredits: {
            used: usage.aiCreditsUsed,
            limit: usage.aiCreditsLimit,
            remaining: Math.max(0, usage.aiCreditsLimit - usage.aiCreditsUsed),
            percent: Math.min(100, Math.round((usage.aiCreditsUsed / usage.aiCreditsLimit) * 100)),
          },
          coverExports: {
            used: usage.coverExportsUsed,
            limit: usage.coverExportsLimit,
            remaining: Math.max(0, usage.coverExportsLimit - usage.coverExportsUsed),
            percent: Math.min(100, Math.round((usage.coverExportsUsed / usage.coverExportsLimit) * 100)),
          },
        },
    ...
    res.status(500).json({
      success: false,
      error: { code: 'DASHBOARD_ERROR', message: err.message || 'Failed to fetch dashboard data.' },
    });
  }
});
 ...
 */
dashboardRouter.post('/quick-action', requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const user = req.user!;
    const { actionType, query } = req.body;

    if (!actionType || typeof actionType !== 'string' || actionType.length > 50) {
       res.status(400).json({ success: false, error: { message: 'Invalid actionType' } });
       return;
    }

    if (query) {
      if (typeof query !== 'string' || query.length > 255) {
        res.status(400).json({ success: false, error: { message: 'Invalid query format or length too long' } });
        return;
      }
      ...
      db.recentSearches.set(searchId, {
        ...
        type: actionType === 'niche' ? 'niche' : actionType === 'book' ? 'book' : 'keyword',
        resultsCount: Math.floor(Math.random() * 500) + 50,
        timestamp: new Date().toISOString(),
      });
    }

    res.json({ success: true, message: `Quick action ${actionType} triggered.` });
  } catch (err: any) {
